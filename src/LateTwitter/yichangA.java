...
public class yichangA {

	
	public static void main(String[] args){
		
	
	//Logger logger =Logger.getLogger(yichangA.class);    //配置日志
	
    Connection conna = ConnectJDBC.getConnection();  
	
    
	PreparedStatement ycsql;
	try {     int i=1;       String  commentgroup = "";
		 ycsql = conna.prepareStatement("select * from yichang");	
	     ResultSet yichs =ycsql.executeQuery();
	    
	     while(yichs.next()) {
		
	    	 String ycid = yichs.getString(1);	     			
     	System.out.println("现在读取的是"+ycid+" 第  "+(i++)+"个用户ID");	
	
	/*	PreparedStatement csql=conna.prepareStatement("select * from twitter where userId = ? ");     	                                    
	     csql.setString(1, ycid);
         ResultSet cssl=csql.executeQuery();	    
                       
         if (cssl.next()) {		
        	 System.out.println("twitter数据表已有"+ycid+"数据！");
        	 continue;              //如果查到了该数据，说明已经访问过，跳过本次for循环；
		}*/
       
         ycid="649782277463891969";

		  //取出异常数据，进行获取跳转id处理	
		try {       
			         // 1、通过HttpGet获取到response对象      
	   HttpClientBuilder    builder2 = HttpClients.custom();  //防止403forbidden的手段。    
       builder2.setUserAgent("Mozilla/5.0(Windows;U;Windows NT 5.1;en-US;rv:0.9.4)");   //设置UA字段，UA标识浏览器身份	       
	 ...
	 HttpGet  httpGet2 = new HttpGet("https://mobile.twitter.com/anyuser/status/"+ycid); 
	      
	 // httpGet2.setConfig(config2); 	       
	 ...
	 System.out.println(response2.getStatusLine().getStatusCode());
			     ...
			     else {System.out.println("此条数据读取失败，连接状态："+response2.getStatusLine().getStatusCode()); 
			    	         int dds=8/0;//如果网络不通返回503，或者被服务端禁止访问返回403则存入erroruser数据库 。
			    	       } 
				        
	                // 2、获取response的entity。  
	            HttpEntity  entity2 = response2.getEntity();  
	            String  rawHtml2 = EntityUtils.toString(entity2);

	        Document   doc2 = Jsoup.parseBodyFragment(rawHtml2);
		
	        Elements  yichangs=doc2.getElementsByClass("timeline inreplytos");
	          
			String[] yics = yichangs.html().split("data-id=\"");
		    String[] yiss =yics[1].split("\">");
			String chus=ycid;     String ss=yiss[0];							
			...
			//	 httpGet.setConfig(config); 	       
                ...
                if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {   }	
			      else {    Thread.currentThread();	Thread.sleep(9000); //如果掉线之类的网络中断或其他，让该线程休眠，放慢读取文件数据的速度
			                     int dds=8/0; System.out.println(dds); } //如果网络不通返回503，或者被服务端禁止访问返回403/529则存入erroruser数据库 。
				 ...
				 Elements   Times =doc.getElementsByClass("metadata");
				   ...
				   //	 System.out.println(time);	                                                         //文章发表时间
				    	 ...
				    	 for (int i1 = 1; i1 < mts.length; i1++) {
					   Document docm = Jsoup.parseBodyFragment(mts[i1]);
			    ...
			    //    	 System.out.println(LastId);
					    ...
					    //####################判断是否有未加载评论############################使用了httpclient技术######################################	
			        	 while(true){
			        		 ...
			        		 // 1、通过HttpGet获取到response对象  
			        		 CloseableHttpClient httpClient3 = builder3.build(); 
			        		 
			        	//	 HttpHost proxy2 = new HttpHost("127.0.0.1", 8118);  
			        	 //    RequestConfig config2 = RequestConfig.custom().setProxy(proxy2).build(); 

					          HttpGet   httpGet3 = new HttpGet("https://mobile.twitter.com/i/rw/conversation/"+ss+"/"+ss+"/descendants/"+LastId); 
					    ...
					    //      httpGet2.setConfig(config2);                   //3.设置代理
							...
							//		System.out.println("此段的评论数量："+(pir1.length-1));						
									...
									for (int i1 = 1; i1 < pir1.length; i1++) {
							           
										if (i1<pir1.length-1){

										    	String   meetjson = "{'html':'"+pir1[i1]+"'}";
								       ...
								       //       	System.out.println(comtext);                          //获取的用户评论内容  
							              	...
							              	if(i1==pir1.length-1){    //获取第二个json数组最后一个用户id        
							              		String   meetjson = "{'html':'"+pir1[i1]+"'}";
								              	JSONObject djson  =new JSONObject(meetjson);		
								              //	System.out.println(djson.get("html"));
		...
		PreparedStatement	 sql=conna.prepareStatement("insert into Twitter values(? ,? , ? ,? ,? )");     	                                    
				         ...
				         System.out.println("已经把"+ss+"数据存储");   
		...
		}catch(Exception e){    //网页连接的try
			e.printStackTrace();
		 try {  	PreparedStatement	errorid = conna.prepareStatement("insert into erroruser2 values( ? )");             
                      errorid.setString(1, ycid);             
	...
	} catch (SQLException e) {	e.printStackTrace();} //error数据库查询try
