package LateTwitter;
	...
	public class twitterERROR implements Runnable {

	public void run() {

	    Logger logger =Logger.getLogger(twitterERROR.class);    //配置日志
	    Connection conna = ConnectJDBC.getConnection();  
	    
                    int zongii = 1  ;           //读取文件
	    try {
		   
		     PreparedStatement csql = null ;  PreparedStatement    ssql =null;  String ss=null;
	
		    PreparedStatement  errorsql =conna.prepareStatement("select * from erroruser2");
		                                   ResultSet  er= errorsql.executeQuery();
		    
			while (er.next()) {
				  try {	
		            	ss =er.getString(1); 		
		            	
					             //同步锁，只能同时有一个进程进入这个判读程序。 1.判断是否已经存在于twitter数据库。
							                                                        //2.如果没有就查询ss表是否被其他线程读过。 3.没有被其他线程读过，就存入ss表。
						//System.out.println("此次读取数据："+zongii++);
					  	 csql=conna.prepareStatement("select userid from twitter  where userId = ? ");     	                                    
						     csql.setString(1, ss);
			                 ResultSet cssl=csql.executeQuery();	    
			                               
			                 if (cssl.next()) {		
			                	 
			                	 System.out.println(Thread.currentThread().getName()+"线程：twitter数据表已有"+ss+"数据！");
			                	 continue;              //如果查到了该数据，说明已经访问过，跳过本次for循环；
							}
                                   System.out.println("shuju:"+ss);
			                 zongii++;                //第多少个用户id
	                      //  logger.warn("读取到了："+ss);
			...
			PreparedStatement	 sql=conna.prepareStatement("insert into Twitter values(? ,? , ? ,? ,?  )");     	                                    
				 ...
				 //一套正常流程已全部走完。*************************	       
	    	            ...
	    	            try {  	PreparedStatement	errorid = conna.prepareStatement("insert into erroruser2 values( ? )");             
	                         errorid.setString(1, ss);             
	                         errorid.executeUpdate();	    		
	                          System.out.println("该条数据在同步区域出错已存入ErrorUser2数据库。");
	                        } catch (SQLException e1) {   e.printStackTrace();	}     
	    	  System.out.println("跳过该数据，不能让线程轻易停止。");
	    	  }//主要是防止同步块出问题。   
	...
	public static void main(String[] args){
		System.out.println("Start~");
		   ...
		   twitterERROR  atw1 =new twitterERROR();
		   twitterERROR  atw2 =new twitterERROR();
		   twitterERROR  atw3 =new twitterERROR();
		   twitterERROR  atw4 =new twitterERROR();
		   twitterERROR  atw5=new twitterERROR();
		   twitterERROR  atw6 =new twitterERROR();
		   twitterERROR  atw7 =new twitterERROR();
		   twitterERROR  atw8 =new twitterERROR();
